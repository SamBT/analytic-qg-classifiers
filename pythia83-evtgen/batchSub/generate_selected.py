...
base_cfg_quark = "-n "+str(evt_per)+" -k "+str(kernel)+" --cf "+str(CF)+" --ca "+str(CA) + " -c "+config_dir+"H2qqbar-ee.cmnd"+" -c "+config_dir+"fixGluonRecoil.cmnd"
base_cfg_gluon = "-n "+str(evt_per)+" -k "+str(kernel)+" --cf "+str(CF)+" --ca "+str(CA) + " -c "+config_dir+"H2gg-ee.cmnd"+" -c "+config_dir+"fixGluonRecoil.cmnd"


#Generating H --> gg samples
for p in prefs:
    for ex in exps:
        name = 'H2gg-CF{0:.1f}CA{1:.1f}-sqg{2}-esg{3}'.format(CF,CA,p,ex)
        cfg_str = base_cfg_gluon + " -c "+config_dir+"splitting-sqg{0}-esg{1}.cmnd".format(p,ex)
