...
base_cfg_quark = "-n "+str(evt_per)+" -k "+str(kernel)+" -r "+str(radius)+" --cf "+str(CF)+" --ca "+str(CA) + \
                " -c "+config_dir+"H2qqbar-ee.cmnd"+" -c "+config_dir+"fixGluonRecoil.cmnd"+" -c "+ \
                config_dir+"splitting-sqg0100.cmnd" + " -e HadronLevel:all=on"
base_cfg_gluon = "-n "+str(evt_per)+" -k "+str(kernel)+" -r "+str(radius)+" --cf "+str(CF)+" --ca "+str(CA) + \
                " -c "+config_dir+"H2gg-ee.cmnd"+" -c "+config_dir+"fixGluonRecoil.cmnd"+" -c "+ \
                config_dir+"splitting-sqg0100.cmnd" + " -e HadronLevel:all=on"

#Generating vanilla H --> gg with the given radius and CF, CA settings
for i in range(1,njobs+1):
    name = "H2gg-hadlevel-CF{0:.1f}CA{1:.1f}-sqg0100-r{2:.1f}".format(CF,CA,radius)
...
for ez in exp_z:
    for ez2 in exp_z2:
        ...
        name = 'H2qq-hadlevel-CF{0:.1f}CA{1:.1f}-sqg0100-esq{2}-r{3:.1f}'.format(CF,CA,esq,radius)
